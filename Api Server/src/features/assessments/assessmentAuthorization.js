...
module.exports.authorizeCompany=async(req,res,next)=>{
    ...
    let assessment=await assessmentsModel.getAssessmentById(assessmentId);
    if(!assessment){
...
module.exports.authorizeCompanyJob=async(req,res,next)=>{
    ...
    let validate=await validateCompanyJob(companyId,jobId);// to check if company is authorized to access this job
    if(!validate){
        return res.status(403).json({
            success: false,
            message: "You are not authorized to access this assessment"
